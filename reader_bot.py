...
async def log_message(message: Message):
    if message.chat.id == CHAT_ID:
        ...
        pattern_start = re.compile(
            r"(на смене|резерв(е)?)\s*\d{2}-\d{2}", re.IGNORECASE
        )
        ...
        if match_start:
            ...
            if 48 <= int(minutes) <= 57 and int(hours) in [
                i for i in range(11, 23) if i % 2
            ]:
                ...
                if not data_list:
                    data_list = await read_output_txt(
                        f"txts/output{today}_{str(int(hours) + 1)}:00.txt"
                    )
                ...
                with open("to_send.txt", "w") as file:
                    ...
                    for data in data_list:
                        file.write(
                            f"{data[0]} | {f'**{data[1]}**' if data[1] else 'Y'} | {f'**{data[2]}**' if data[2] else 'Y'}\n\n"
                        )

        if match_stop:
            # validation for 14:00 - 14:10
            if 0 <= int(minutes) >= 10 and int(hours) in [
                hour for hour in range(14, 24, 2)
            ]:
                data_list = []
                ...
                if not data_list:
                    data_list = await read_output_txt(
                        f"txts/output{today}_{str(int(hours))}:00.txt"
                    )
                data_list = [list(x) for x in data_list]
                try:
                    data_list2 = await read_output_txt(
                        f"txts/output{today}_{str(int(hours) + 2)}:00.txt"
                    )
